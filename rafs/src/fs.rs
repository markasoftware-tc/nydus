...
impl Rafs {
    ...
    pub fn new(cfg: &Arc<ConfigV2>, id: &str, path: &Path) -> RafsResult<(Self, RafsIoReader)> {
        ...
        let rafs = Rafs {
            ...
            prefetch_all_batch_size: rafs_cfg.prefetch.prefetch_all_batch_size as u64,
...
impl Rafs {
    fn prefetch(&self, reader: RafsIoReader, prefetch_files: Option<Vec<PathBuf>>) {
        ...
        let prefetch_all_batch_size = self.prefetch_all_batch_size;
        let root_ino = self.root_ino();

        let _ = std::thread::spawn(move || {
            Self::do_prefetch(root_ino, reader, prefetch_files, prefetch_all, prefetch_all_batch_size, sb, device);
        });
    }
    ...
    fn do_prefetch(
        ...
        if !ignore_prefetch_all && (inlay_prefetch_all || prefetch_all || startup_prefetch_all) {
            if sb.meta.is_v6() {
                ...
                let batch_size = prefetch_all_batch_size;
